...
class ThreatTimelinePipeline:
    def __init__(self):
        ...
        self.mitre_embeddings = self.model.encode(
            self.mitre_df["description"].tolist(), batch_size=64, convert_to_numpy=True
        )
    ...
    def build_cpe(self, row: pd.Series) -> str:
        part = {"a": "a", "o": "o", "h": "h"}.get(
            str(row.get("part", "a")).lower(), "a"
        )
    ...
    def get_nvd_cves_enhanced(self, cpe: str) -> List[Dict]:
        ...
        try:
            ...
            for vuln in data.get("vulnerabilities", [])[:5]:
                ...
                cvss_v3 = (
                    cvss_v3_metrics[0].get("cvssData", {}).get("baseScore")
                    if cvss_v3_metrics
                    else None
                )
                cvss_v4 = (
                    cvss_v4_metrics[0].get("cvssData", {}).get("baseScore")
                    if cvss_v4_metrics
                    else None
                )
                cvss_v2 = (
                    cvss_v2_metrics[0].get("cvssData", {}).get("baseScore")
                    if cvss_v2_metrics
                    else None
                )

                nvd_severity = self._get_nvd_severity(cvss_v3 or cvss_v4 or cvss_v2)
                nvd_severity_score = self._get_nvd_severity_score(nvd_severity)

                v3_vector = (
                    cvss_v3_metrics[0].get("cvssData", {}).get("vectorString")
                    if cvss_v3_metrics
                    else None
                )
                v4_vector = (
                    cvss_v4_metrics[0].get("cvssData", {}).get("vectorString")
                    if cvss_v4_metrics
                    else None
                )
    ...
    def run_pipeline(self, input_csv: str, output_csv: str):
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = [
                executor.submit(self.process_asset, row) for _, row in df.iterrows()
            ]
                ...
                result.update(
                    {
                        ...
                        "attack_vector_detail": (
                            mitres[0]["tactic_detail"] if mitres else None
                        ),
                        "mitre_top1": mitres[0]["technique_id"] if mitres else None,
                        "mitre_top1_name": (
                            mitres[0]["technique_name"] if mitres else None
                        ),
        ...
        if all_results:
            ...
            df_out = df_out[[col for col in timeline_cols if col in df_out.columns]]
            df_out.to_csv(output_csv, index=False)

        elapsed = time.time() - start
        high_severity = (
            len(df_out[df_out["nvd_severity"] == "HIGH"]) if "df_out" in locals() else 0
        )
        critical = (
            len(df_out[df_out["nvd_severity"] == "CRITICAL"])
            if "df_out" in locals()
            else 0
        )
